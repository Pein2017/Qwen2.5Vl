...
def _build_minimal_training_config(base_model) -> Any:
    ...
    merge_size = 14
    try:
        if hasattr(base_model.config, "vision_config") and getattr(base_model.config.vision_config, "spatial_merge_size", None) is not None:
...
def load_detection_model(checkpoint: str, processor: Qwen2VLProcessor, device: str) -> DetectionModel:
    ...
    model = DetectionModel(base_model=base_model, config=train_cfg, tokenizer=processor.tokenizer, skip_expansion=True)
...
def _get_lang_layers(module: torch.nn.Module):
    for attr in ["model", "language_model", "transformer"]:
        ...
        if sub is None:
            continue
        layers = getattr(sub, "layers", None) or getattr(sub, "h", None) or getattr(sub, "decoder", None)
...
def _get_vision_blocks(module: torch.nn.Module):
    ...
    for attr in ["visual", "vision_tower", "vision_model"]:
        ...
        if sub is not None:
            blocks = getattr(sub, "blocks", None) or getattr(sub, "layers", None) or getattr(sub, "encoder", None)
...
def apply_freeze_and_param_groups(policy: DetectionModel, tokenizer, cfg) -> List[Dict[str, Any]]:
    # Freeze all first
    ...
    if len(params_groups) == 0:
        ...
        if fallback:
            params_groups = [{"params": fallback, "lr": cfg.learning_rate, "name": "default"}]
