...
class BaseCreditAssigner:
    def compute_loss_a(
        ...
        tf_cfg: Dict[str, Any],  # length_norm, logits_processors, stopping, device, K_A, max_images_tf, ddp_policy
        gen_cfg_a: GenerationConfig,
        reward_cfg: Dict[str, Any],  # names, weights, group_reward_mode, use_ref_kl, ref_model, lambda_kl_stage_a, use_uncertainty_gate, entropy_threshold, use_mission_checklist, baseline_tf_p_pass, baseline_tf_p_fail
...
class ConditionalAssigner(BaseCreditAssigner):
    def compute_loss_a(
        ...
        group_reward_mode: str = str(reward_cfg.get("group_reward_mode", "combined")).lower()
        ...
        use_mission_checklist: bool = bool(reward_cfg.get("use_mission_checklist", True))
        ...
        baseline_margin = decision_margin(enc_b_baseline, processor, train_model, length_norm=length_norm)
        ...
        if entropy_vals:
            diags["phase_a_entropy_mean"] = float(sum(entropy_vals) / max(1, len(entropy_vals)))
...
class PairwiseFallbackAssigner(BaseCreditAssigner):
    def compute_loss_a(
        ...
        eps = 1e-9
        r_baseline = _math.log(max(eps, float(reward_cfg.get("baseline_tf_p_pass", 0.0))) + eps) - _math.log(max(eps, float(reward_cfg.get("baseline_tf_p_fail", 0.0))) + eps)
        for (i, j) in pairs:
            # i
            msgs_i = conv_builder.build_stage_a_messages(mission)
            text_ai = processor.apply_chat_template(msgs_i, tokenize=False, add_generation_prompt=True)
            GroupQCConversationBuilder.validate_typed_image_count(msgs_i, 1)
            enc_ai = processor(text=[text_ai], images=[sft_style_preprocess_image(images[i])], padding=True, return_tensors="pt")
            ...
            text_aj = processor.apply_chat_template(msgs_j, tokenize=False, add_generation_prompt=True)
            GroupQCConversationBuilder.validate_typed_image_count(msgs_j, 1)
            enc_aj = processor(text=[text_aj], images=[sft_style_preprocess_image(images[j])], padding=True, return_tensors="pt")
            ...
            if use_mission_checklist:
                msgs_b = conv_builder.build_stage_b_messages(summary_lines=context_pair, checklist_lines=checklist)
            else:
                msgs_b = conv_builder.build_stage_b_messages_minimal(summary_lines=context_pair)
            ...
            try:
                p_pass, p_fail = tf_decision_probs(train_model, enc_b, processor, length_norm=length_norm)
            except Exception:
            ...
            mean = rt.mean(); std = rt.std(unbiased=False)
            if float(std.item()) < 1e-6:
            ...
            for enc_a_cand, toks_cand in ((enc_ai, toks_i), (enc_aj, toks_j)):
                ...
                if use_ref_kl and (ref_model is not None) and (lambda_kl_stage_a > 0.0):
                    kl_k = kl_to_ref_with_cur_logits(ref_model, enc_a_cand, toks_cand, cur_logits_k, int(prompt_len_k))
