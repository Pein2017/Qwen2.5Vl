...
def aggregate_training_metrics(vec_local: torch.Tensor, world_size: int) -> AggregatedMetrics:
    ...
    Expected vec_local layout:
        [ total_items,
          ...
          kl_b_sum,
          kl_b_count,
          # Phase‑A diagnostics (sums over items; optional)
          diag_fmt_sum,
          diag_cov_sum,
          diag_cln_sum,
          diag_tax_sum,
    ...
    (
        ...
        fn_count_g,
        gt_fail_count_g,
    ) = [float(x) for x in vec_local.tolist()]
    ...
    # Diagnostics means (if provided; else zeros)
    phase_a_formatting = diag_fmt_sum_g / tot_items_g
    phase_a_coverage = diag_cov_sum_g / tot_items_g
    phase_a_cleanliness = diag_cln_sum_g / tot_items_g
    phase_a_taxonomy = diag_tax_sum_g / tot_items_g
    phase_a_consistency = diag_cons_sum_g / tot_items_g
    # Rates
    ...
    out: AggregatedMetrics = {
        ...
        # Diagnostics (Stage‑A)
        "phase_a_formatting": float(phase_a_formatting),
        "phase_a_coverage": float(phase_a_coverage),
        "phase_a_cleanliness": float(phase_a_cleanliness),
        "phase_a_taxonomy": float(phase_a_taxonomy),
        "phase_a_consistency": float(phase_a_consistency),
        # Rates
...
def rank0_log(
    ...
    # Always prefer raw metrics; if a windowed value is present, use it as a fallback display
    def pick(key: str, default: float = 0.0) -> float:
        return float(scalars.get(key, scalars.get(f"{key}_win", default)))
    ...
    if tb_writer is not None:
        try:
            for k in [
                "train/loss",
                "train/reward_best_mean",
                "train/reward_best_std",
                "train/acc_best",
                "train/fn_rate",
                "train/resp_len_mean",
                "train/grad_norm_mean",
                "train/eta_hours",
                "train/reward_margin_best",
                "train/decision_ce_mean",
            ]:
                tb_key = k.split("/", 1)[1]
                if tb_key in scalars:
                    tb_writer.add_scalar(k, float(scalars[tb_key]), step)
            if 'kl_b_mean' in scalars:
