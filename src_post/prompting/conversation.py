...
from src_new.processing.templates import SUMMARY_SYSTEM_PROMPT, SUMMARY_USER_PROMPT
...
STAGE_B_SYSTEM_PROMPT: str = (
    ...
    # "判定准则（宽松、默认通过）：若摘要未出现与本任务直接相关的明确负面项（例如：未拧紧/露铜/复接/生锈/弯曲半径不合理/分布散乱/安装方向错误），默认判定为通过。\n"
    # "“标签/无法识别”不单独作为不通过依据；仅当摘要出现明确负面项时，才可判定为不通过。\n"
    "输出格式严格为两行：\n"
    "  第一行：总评: 通过 或 总评: 不通过\n"
    "  第二行：原因: <1–2条来自摘要的关键依据>（必须简洁、具体，且与本任务直接相关；不得凭空引入新的负面项）\n"
...
MISSION_HINTS: Dict[str, List[str]] = {
    "bbu安装方式检查": [
        "挡风板需求与符合性：机柜空间充足需要安装 / 无需安装；如需安装，则应为‘这个BBU设备按要求配备了挡风板’，否则为‘这个BBU设备未按要求配备挡风板’",
        "连接点合规与细项：不符合要求（未拧紧/露铜/复接/生锈）",
        "电线整齐度：捆扎整齐 / 分布散乱",
        "光纤弯曲半径：弯曲半径合理 / 弯曲半径不合理（弯曲半径<4cm或者成环）",
        "标签可读性：仅在无法识别时输出‘标签/无法识别’（可识别时不输出标签项）",
    ],
    "bbu接地线检查": [
        "接地连接是否规范：不符合要求（未拧紧/露铜/复接/生锈）；尤其关注‘露铜’",
        "标签可读性：仅在无法识别时输出‘标签/无法识别’（可识别时不输出标签项）",
    ],
    "bbu线缆布放要求": [
        "接地连接是否规范（机柜/地排处）：不符合要求（未拧紧/复接/生锈）",
        "光纤保护措施：无保护措施 / 有保护措施（蛇形管/铠装/同时有蛇形管和铠装）",
        "光纤弯曲半径：弯曲半径合理 / 弯曲半径不合理（弯曲半径<4cm或者成环）",
        "电线整齐度：捆扎整齐 / 分布散乱",
        "关键连接点标签可读性：仅在无法识别时输出‘标签/无法识别’（可识别时不输出标签项）",
    ],
    "挡风板安装检查": [
        "挡风板需求与符合性：机柜空间充足需要安装；如需安装，则应为‘这个BBU设备按要求配备了挡风板’，否则为‘这个BBU设备未按要求配备挡风板’",
