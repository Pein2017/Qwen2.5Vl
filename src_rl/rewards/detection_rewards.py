...
from src_new.augmentation.utils import bbox_from_quad_flat, iou_bbox
...
_BOX_PATTERN = re.compile(
    r"<\|(?:object_ref_start|obj_ref_start)\|>(.*?)<\|(?:object_ref_end|obj_ref_end)\|>\s*"
...
def _extract_ints(section: str) -> List[int]:
    return [int(match) for match in re.findall(r"-?\d+", section)]


def _clean_desc(text: str) -> str:
    cleaned = text.strip()
    cleaned = cleaned.replace("<|object_ref_start|>", "").replace("<|object_ref_end|>", "")
...
def parse_dense_caption(text: str) -> List[Dict[str, object]]:
    ...
    for pattern, key in ((_BOX_PATTERN, "bbox_2d"), (_QUAD_PATTERN, "quad"), (_LINE_PATTERN, "line")):
        for match in pattern.finditer(text):
            desc = _clean_desc(match.group(1))
            coords = _extract_ints(match.group(2))
            if key == "bbox_2d" and len(coords) >= 4:
                coords = coords[:4]
            elif key == "quad" and len(coords) >= 8:
                coords = coords[:8]
            elif key == "line" and (len(coords) < 4 or len(coords) % 2 != 0):
                continue
            obj = {"desc": desc}
            obj[key] = coords
            objects.append(obj)
...
def _iter_gt_boxes(objects: Iterable[Dict[str, object]]) -> List[Tuple[int, int, int, int]]:
...
def _intersection(box_a: Tuple[int, int, int, int], box_b: Tuple[int, int, int, int]) -> float:
...
def _enclosing(box_a: Tuple[int, int, int, int], box_b: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
    ax1, ay1, ax2, ay2 = box_a
    bx1, by1, bx2, by2 = box_b
    cx1 = min(ax1, bx1)
    cy1 = min(ay1, by1)
...
def giou_bbox(box_a: Tuple[int, int, int, int], box_b: Tuple[int, int, int, int]) -> float:
...
def _normalization_scale(meta: Optional[Dict[str, object]], max_coord_value: Optional[int]) -> float:
    """Choose a reasonable normalization scale for L1-like geometry errors."""
    candidates: List[int] = []
    if isinstance(max_coord_value, (int, float)):
        candidates.append(int(max_coord_value))
...
def _greedy_match(preds: List[List[int]], gts: List[List[int]], dist_fn) -> List[Tuple[int, int]]:
...
def _assign_pairs(preds: List[List[int]] | List[Tuple[int, int, int, int]],
                  gts: List[List[int]] | List[Tuple[int, int, int, int]],
                  cost_fn) -> List[Tuple[int, int]]:
    ...
    return _greedy_match(list(range(len(preds))), list(range(len(gts))), lambda pi, gi: _dist_from_matrix(pi, gi))


# ------------------- Rewards -------------------

def reward_coverage(text: str, *, meta: Optional[Dict[str, object]] = None, **_: object) -> float:
    ...
    **_: object,
    ...
    if isinstance(meta, dict):
        ...
        if max_coord_value is None:
            max_coord_value = meta.get("max_coord_value")

    limit = None
    values = [max_coord_value, width, height]
...
def reward_bbox_iou(
    ...
    **_: object,
    ...
    def _mapped_giou(a: Tuple[int, int, int, int], b: Tuple[int, int, int, int]) -> float:
    ...
    **_: object,
    ...
    if isinstance(meta, dict):
        for o in meta.get("objects", []) or []:
            if isinstance(o, dict) and "quad" in o and isinstance(o["quad"], list) and len(o["quad"]) >= 8:
    ...
    scale = _normalization_scale(meta, max_coord_value)
...
def reward_line_l1(
    ...
    **_: object,
    ...
    for o in pred_objs:
        if "line" in o and isinstance(o["line"], list) and len(o["line"]) >= 4 and len(o["line"]) % 2 == 0:
    ...
    if isinstance(meta, dict):
        for o in meta.get("objects", []) or []:
            if isinstance(o, dict) and "line" in o and isinstance(o["line"], list) and len(o["line"]) >= 4 and len(o["line"]) % 2 == 0:
    ...
    def _endpoints(points_flat: List[int]) -> List[int]:
        pts = [(int(points_flat[i]), int(points_flat[i + 1])) for i in range(0, len(points_flat), 2)]
