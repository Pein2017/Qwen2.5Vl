...
from src_rl.rewards.detection_rewards import (
    parse_dense_caption,
    reward_bbox_giou,
    reward_coverage,
...
def test_reward_quad_l1_and_ordering():
    ...
    assert reward_quad_l1(text, meta=meta, max_coord_value=100) == 1.0
...
def test_reward_line_l1_and_ordering():
    ...
    assert reward_line_l1(text, meta=meta, max_coord_value=100) == 1.0
    assert reward_ordering(text) == 1.0
