#!/usr/bin/env python3
"""
Reference Architecture Training Script - Using BBUTrainer (src_new_reference)

This training script mirrors scripts/train_new.py but is wired to the
src_new_reference pipeline, which uses reference-style SFT labeling
(single CE over labels; no spans/grouping) and compatible components.

Usage:
    python scripts/train_new_ref.py --config phase_2/standard --log_level INFO
"""
...
from src_new_reference.utils.seeding import seed_everything
...
try:
    from src_new_reference.utils.rank_aware_logging import initialize_logging_from_env
...
from src_new_reference.models.patches import apply_comprehensive_qwen25_fixes
...
if TYPE_CHECKING:
    ...
    from src_new_reference.config.config import Config
...
def get_logger():
    ...
    from src_new_reference.utils.logger_factory import get_training_logger

    return get_training_logger("train_new_ref")


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Reference Architecture Training Script (src_new_reference)")

    # Core arguments
    parser.add_argument("--config", required=True, help="Config name under configs/ (e.g., 'phase_2/standard')")
...
def create_trainer_with_new_architecture(
    ...
    from src_new_reference.data.collator import create_data_collator
    from src_new_reference.data.dataset import Dataset
    from src_new_reference.data.teacher_pool import TeacherPoolManager
    from src_new_reference.models.wrapper import DetectionModel

    # Phase-based freezing manager for separate runs
    try:
        from src_new_reference.training.phase_freeze_manager import PhaseFreezeManager
    except Exception:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        use_fast=True,  # fast tokenizer for offsets/validation
    )

    # Load image processor separately (following pattern)
    ...
    # Disable augmentation for validation dataset by default
    ...
    from src_new_reference.training.bbu_trainer import BBUTrainer
    ...
    try:
        from src_new_reference.training.callbacks import AugmentationScheduleCallback
...
def main():
    from src_new_reference.config.config import load_config, set_global_log_level
