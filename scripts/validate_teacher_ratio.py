...
def test_teacher_ratio_distribution(num_samples: int = 1000):
    ...
    if deviation <= tolerance:
        print(
            f"✅ PASS: Teacher ratio within acceptable tolerance ({deviation:.3f} <= {tolerance})"
        )
        success = True
    else:
        print(
            f"❌ FAIL: Teacher ratio deviation too high ({deviation:.3f} > {tolerance})"
        )
        success = False
    ...
    if summary["total_samples_processed"] != processed_samples:
        print(
            f"⚠️  Warning: Sample count mismatch ({summary['total_samples_processed']} vs {processed_samples})"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate teacher ratio implementation"
    )
    parser.add_argument("--config", default="base_flat_v2", help="Config name")
    parser.add_argument(
        "--samples", type=int, default=1000, help="Number of samples to test"
    )
