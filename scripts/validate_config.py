                    ...
                    try:
                        ...
                        if "objects" in sample and sample["objects"]:
                            ...
                            if not coordinate_tokens_enabled:
                                issues.append(
                                    f"⚠️  INFO: {data_file} contains detection data but coordinate tokens not enabled"
                                )
...
def check_training_parameters():
    ...
    if hasattr(config, "learning_rate") and hasattr(config, "adapter_lr"):
        if config.learning_rate == 0 and config.adapter_lr > 0:
            issues.append(
                f"ℹ️  Using adapter-only training (LR=0, adapter_lr={config.adapter_lr})"
            )
    ...
    if getattr(config, "coordinate_config_enable_coordinate_tokens", False):
        required_params = [
            "coordinate_tokens_max_coord_value",
            "coordinate_tokens_temperature",
        ]
        for param in required_params:
            if not hasattr(config, param):
                issues.append(
                    f"❌ Missing coordinate token parameter: {param} (using defaults)"
                )
            elif getattr(config, param) <= 0:
                issues.append(
                    f"⚠️  Detection parameter {param} = {getattr(config, param)} (should be > 0)"
                )
...
def main():
    ...
    parser.add_argument(
        "--config", required=True, help="Config name (e.g., base_flat_v2)"
    )
    ...
    try:
        ...
        if not all_issues:
            ...
            critical_issues = [
                issue for issue in all_issues if "CRITICAL" in issue or "❌" in issue
            ]
            if critical_issues:
        ...
        print(
            f"   Coordinate tokens enabled: {getattr(config, 'coordinate_config_enable_coordinate_tokens', False)}"
        )
