...
def get_bboxes_and_dims(sample: dict):
    ...
    if isinstance(objs, dict) and 'bbox' in objs:
        ...
        bboxes = [item['box'] for item in objs if isinstance(item, dict) and 'box' in item]
    else:
        bboxes = []
    # get image dims
...
def main():
    ...
    parser.add_argument(
        '--hierarchy', type=str, required=True,
        help='Path to label_hierarchy.json'
    )
    parser.add_argument(
        '--max_teachers', type=int, default=10,
        help='Maximum number of teacher samples to select'
    )
    parser.add_argument(
        '--output', type=str, default='data_analysis/teacher_pool.json',
        help='Output JSON file to save selected teacher image paths'
    )
    parser.add_argument(
        '--seed', type=int, default=42,
        help='Random seed for final pool fill'
    )
    ...
    need_spatial = set(item['spatial_bucket'] for item in info_list if item['spatial_bucket'] != 'unknown')
    need_sizes = set(item['size_bucket'] for item in info_list if item['size_bucket'] != 'unknown')
    teacher_indices: list[int] = []
    while (need_labels or need_counts or need_spatial or need_sizes) and len(teacher_indices) < args.max_teachers:
        best_score = -1
        best_info = None
    ...
    while len(teacher_indices) < args.max_teachers and remaining_infos:
        ...
        remaining_infos = [info for info in remaining_infos if info['idx'] != choice['idx']]

    # Prepare output: image paths from target
    teacher_paths: list[str] = []
    for idx in teacher_indices:
        sample = samples[idx]
        target = sample.get('target', sample)
        imgs = target.get('images')
        if isinstance(imgs, list) and imgs:
            teacher_paths.append(imgs[0])

    # Write to output JSON
    out_path = Path(args.output)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(teacher_paths, f, ensure_ascii=False, indent=2)

    print(f"Selected {len(teacher_paths)} teacher samples; saved to {args.output}")
