...
class TeacherSelector:
    ...
    def _get_geometry_types_bucket(self, sample: Dict) -> str:
        """Categorize sample by geometry types present."""
        objects = sample.get("objects", [])
        ...
        # Categorize by geometry diversity
        if len(geometry_types) == 1:
            # Single geometry type
            geom_type = list(geometry_types)[0]
            if geom_type == "bbox":
                return "simple_bbox"
            elif geom_type == "ExtentPolygon":
                ...
                return f"simple_{geom_type.lower()}"
        elif len(geometry_types) == 2:
            return "mixed_dual"
        else:
            return "mixed_complex"

    def _get_path_complexity_bucket(self, sample: Dict) -> str:
        """Categorize sample by path/line complexity."""
        objects = sample.get("objects", [])
        ...
        line_objects = []
        for obj in objects:
            if "geometry" in obj and obj["geometry"].get("type") == "LineString":
        ...
        for obj in line_objects:
            points = CoordinateManager.get_all_coordinate_points(obj["geometry"])
        ...
        avg_points_per_path = total_points / len(line_objects)
        avg_path_length = sum(path_lengths) / len(path_lengths)

        # Categorize by complexity
        if avg_points_per_path <= 3 and avg_path_length < 100:
            return "simple_paths"
        elif avg_points_per_path <= 6 and avg_path_length < 500:
            return "medium_paths"
        else:
    ...
    def _compute_sample_metadata(self, samples: List[Dict]) -> List[Dict]:
        ...
        for idx, sample in enumerate(samples):
            ...
            geometry_bucket = self._get_geometry_types_bucket(sample)
            path_bucket = self._get_path_complexity_bucket(sample)
    ...
    def _greedy_selection(self, metadata_list: List[Dict]) -> List[int]:
        ...
        needed_geometry_buckets = set(
            item["geometry_bucket"]
            for item in metadata_list
            if item["geometry_bucket"] != "no_objects"
        )
        needed_path_buckets = set(
            item["path_bucket"]
            for item in metadata_list
            if item["path_bucket"] != "no_paths"
        )

        logger.debug(
            f"Initial coverage needs: {len(needed_labels)} labels, "
            ...
            f"{len(needed_geometry_buckets)} geometry buckets, "
            f"{len(needed_path_buckets)} path buckets"
        )

        # Greedy selection loop
        while (
            needed_labels
            or needed_count_buckets
            or needed_spatial_buckets
            or needed_size_buckets
            or needed_geometry_buckets
            or needed_path_buckets
        ) and len(selected_indices) < self.max_teachers:
            best_score = -1
            best_metadata = None
            ...
            for metadata in metadata_list:
                ...
                count_gain = (
                    1 if metadata["count_bucket"] in needed_count_buckets else 0
                )
                spatial_gain = (
                    1 if metadata["spatial_bucket"] in needed_spatial_buckets else 0
                )
                size_gain = 1 if metadata["size_bucket"] in needed_size_buckets else 0
                geometry_gain = (
                    1 if metadata["geometry_bucket"] in needed_geometry_buckets else 0
                )
                path_gain = 1 if metadata["path_bucket"] in needed_path_buckets else 0

                # Weighted score (prioritize label coverage, then geometry diversity)
                score = (
                    10 * label_gain
                    + 4 * geometry_gain
                    + 3 * count_gain
            ...
            if not best_metadata or best_score <= 0:
                logger.debug(
                    "No more beneficial samples found, stopping greedy selection"
                )
                break
            ...
            needed_geometry_buckets.discard(best_metadata["geometry_bucket"])
            needed_path_buckets.discard(best_metadata["path_bucket"])

            logger.debug(
                f"Selected sample {best_metadata['idx']} "
                f"(score={best_score}, labels={len(best_metadata['labels'])}, "
                f"geometry={best_metadata['geometry_bucket']}, "
                f"path={best_metadata['path_bucket']}, "
    ...
    def _random_fill(
        self, selected_indices: List[int], metadata_list: List[Dict]
    ) -> List[int]:
        ...
        remaining_metadata = [
            m for m in metadata_list if m["idx"] not in selected_indices
        ]
        ...
        while len(selected_indices) < self.max_teachers and remaining_metadata:
            ...
            remaining_metadata = [
                m for m in remaining_metadata if m["idx"] != choice["idx"]
            ]
    ...
    def select_teachers(self, samples: List[Dict]) -> Tuple[List[Dict], List[int]]:
        ...
        if len(samples) <= self.max_teachers:
            ...
            if len(samples) <= 2:
                ...
                logger.info(
                    f"Very few samples ({len(samples)}), selecting 0 teachers to preserve training data"
                )
                return [], []
            else:
                ...
                logger.info(
                    f"Few samples ({len(samples)}), selecting {selected_count} teachers, keeping 1 for training"
                )
                return samples[:selected_count], list(range(selected_count))

        logger.info(
            f"Selecting {self.max_teachers} teachers from {len(samples)} samples"
        )
