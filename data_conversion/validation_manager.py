...
class ValidationManager:
    ...
    def _validate_sample_structure(self, sample: Dict[str, Any], report: ValidationReport) -> None:
    ...
    def _validate_object(
        ...
        for geom_type in geometry_types:
            if geom_type in obj:
                ...
                self._validate_geometry(obj[geom_type], geom_type, object_index, report, image_width, image_height)
        
        if found_geometry is None:
            report.add_error(ValidationError(
                error_type="missing_geometry",
                severity="critical",
                message=f"Object {object_index} missing geometry (bbox_2d, square, or line)",
                fix_suggestion="Add one of: bbox_2d, square, or line geometry",
                object_index=object_index
            ))

    def _validate_description(self, desc: Any, object_index: int, report: ValidationReport) -> None:
        ...
        if self.require_non_empty_description and not desc.strip():
            report.add_error(ValidationError(
                ...
                severity="warning" if self.validation_mode == "lenient" else "critical",
    ...
    def _validate_geometry(
        ...
        if geom_type == "bbox_2d":
            self._validate_bbox(geometry, object_index, report, image_width, image_height)
        elif geom_type == "square":
            self._validate_square(geometry, object_index, report, image_width, image_height)
        elif geom_type == "line":
            self._validate_line(geometry, object_index, report, image_width, image_height)
    ...
    def _validate_square(
        ...
        if not isinstance(square, list) or len(square) != 8:
            report.add_error(ValidationError(
                error_type="invalid_square_format",
                severity="critical",
                message=f"Object {object_index} square must be list of 8 coordinates",
                fix_suggestion="Ensure square is [x1,y1,x2,y2,x3,y3,x4,y4]",
                object_index=object_index,
                field_name="square",
                invalid_value=square
            ))
            return
        ...
        for i, coord in enumerate(square):
            if not isinstance(coord, (int, float)):
                report.add_error(ValidationError(
                    error_type="invalid_coordinate_type",
                    severity="critical",
                    message=f"Object {object_index} square coordinate {i} must be number",
                    fix_suggestion="Ensure all coordinates are numeric values",
                    object_index=object_index,
                    field_name=f"square[{i}]",
                    invalid_value=coord
                ))
                return
        ...
        if self.check_coordinate_bounds and image_width and image_height:
            for i in range(0, 8, 2):
                x, y = square[i], square[i+1]
                if x < 0 or y < 0 or x > image_width or y > image_height:
                    report.add_error(ValidationError(
                        error_type="coordinates_out_of_bounds",
                        severity="critical",
                        message=f"Object {object_index} square point {i//2} out of bounds",
                        fix_suggestion=f"Ensure coordinates are within image ({image_width}x{image_height})",
                        object_index=object_index,
                        field_name="square",
    ...
    def filter_valid_objects(
        ...
        for i, obj in enumerate(objects):
            ...
            is_valid, report = self.validate_sample(temp_sample, f"{sample_id}_obj_{i}", image_width, image_height)
            ...
            if is_valid or self.validation_mode == "warning_only":
                ...
                invalid_obj["_validation_errors"] = [error.to_dict() for error in report.errors]
    ...
    def generate_validation_summary(self) -> Dict[str, Any]:
        ...
        return {
            "validation_summary": {
                ...
                "validation_success_rate": self.valid_samples / max(1, self.total_samples_processed),
    ...
    def export_validation_reports(self, output_dir: Path) -> Dict[str, str]:
        ...
        invalid_reports = [report.to_dict() for report in self.validation_reports if not report.is_valid]
