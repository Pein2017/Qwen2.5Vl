...
def _summarize_object_string(desc: str) -> str:
    ...
    if kind == CONNECT and len(parts) >= 2:
        ...
        if CP_COMPLY_BAD in lvl1:
            ...
            if issues:
                return "/".join([p for p in ["螺丝、光纤插头", completeness, f"不符合要求/{','.join(sorted(set(issues)))}"] if p])
            return "/".join([p for p in ["螺丝、光纤插头", completeness, "不符合要求"] if p])
        if CP_COMPLY_OK in lvl1:
            return "/".join([p for p in ["螺丝、光纤插头", completeness, "符合要求"] if p])
        return ""
