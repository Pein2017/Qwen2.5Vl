...
class PipelineStep:
    ...
    def complete(self, success: bool = True, error_message: Optional[str] = None) -> None:
        ...
        if success:
            ...
            logger.error(f"❌ Step {self.name} failed: {error_message} ({duration:.2f}s)")
...
class PipelineManager:
    ...
    def clean_raw_json(self) -> bool:
        ...
        try:
            ...
            output_dir = Path(self.config.output_image_dir) if self.config.output_image_dir else input_dir
    ...
    def apply_token_mapping(self) -> bool:
        ...
        try:
            ...
            output_dir = Path(self.config.output_image_dir) if self.config.output_image_dir else Path(self.config.input_dir)
            ...
            for json_file in json_files:
                ...
                if "markResult" in data and "features" in data["markResult"]:
                    for feature in data["markResult"]["features"]:
                        if "properties" in feature and "contentZh" in feature["properties"]:
                            content_zh = feature["properties"]["contentZh"]
                            updated_content = token_mapper.apply_to_content_zh(content_zh)
    ...
    def process_samples(self) -> Optional[Dict[str, int]]:
        ...
        try:
            ...
            if self.config.output_image_dir and Path(self.config.output_image_dir).exists():
    ...
    def validate_output(self) -> bool:
        ...
        try:
            ...
            required_files = ["train.jsonl", "val.jsonl", "teacher.jsonl", "all_samples.jsonl"]
            ...
            expected_total = counts["train.jsonl"] + counts["val.jsonl"] + counts["teacher.jsonl"]
    ...
    def generate_summary(self, processing_result: Optional[Dict[str, int]]) -> bool:
        ...
        try:
            ...
            total_duration = sum(s.get_duration() for s in self.steps[:-1])  # Exclude current step
    ...
    def _process_images_and_update_json(self, input_dir: Path, output_dir: Path) -> None:
        ...
        for ext in ["*.jpeg", "*.jpg"]:
            for image_file in input_dir.glob(ext):
                try:
                    ...
                    original_width, original_height = FileOperations.get_image_dimensions(image_file)
                    
                    # Process image (copy or resize)
                    output_image_path, final_width, final_height = image_processor.process_image(
                        image_file, original_width, original_height, output_dir.parent
                    )
                    ...
                    if (final_width != original_width or final_height != original_height):
                        ...
                        if json_file.exists():
                            ...
                            if "info" in json_data:
                                ...
                                if "markResult" in json_data and "features" in json_data["markResult"]:
                                ...
                                logger.debug(f"Updated JSON dimensions for {json_file.name}: {original_width}x{original_height} → {final_width}x{final_height}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unified Data Conversion Pipeline Manager")
    
    # Configuration options
    parser.add_argument("--input_dir", default="ds", help="Input directory with JSON/image files")
    ...
    parser.add_argument("--language", choices=["chinese", "english"], default="chinese", help="Language mode")
    ...
    parser.add_argument("--from_env", action="store_true", 
