...
def trim_at_first_im_end(text: str, tokenizer: Any | None = None) -> str:
    """Return ``text`` trimmed at the first ``<|im_end|>`` occurrence.

    The tokenizer is optional and only present for signature parity if future
    variants need to resolve token forms dynamically.
    """
