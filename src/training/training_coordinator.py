...
class TrainingCoordinator:
    ...
    def __init__(
        ...
        # Initialize managers - parameter management moved to TrainingStateManager
        self.loss_manager = self._create_loss_manager()

        # Training state - all from typed configs
        self.current_epoch = 0
        self.global_step = 0
        self.detection_training_enabled = (
            self.coordinate_config.coordinate_tokens_enabled
        )
    ...
    def _create_loss_manager(self) -> LossManager:
        ...
        return LossManager(
            config=self.coordinate_config,  # Pass coordinate config that has coordinate_tokens_enabled
    ...
    def setup_training(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with training setup info
        """
        # Log training setup
        self.logger.info("🚀 Training coordinator setup completed:")
        self.logger.info(
    ...
    def get_averaged_losses_and_reset(self) -> Dict[str, float]:
        ...
        # UNIFIED RESET - Reset loss accumulation after averaging
        self.loss_manager.reset_loss_accumulation()
    ...
    def get_component_gradients(self) -> Dict[str, float]:
        """Get gradient norms for each component (delegated to TrainingStateManager)."""
        # This functionality is now handled by TrainingStateManager
        return {}
    ...
    def freeze_components(self, components: List[str]):
        """Freeze specified model components (delegated to TrainingStateManager)."""
        # This functionality is now handled by TrainingStateManager
        self.logger.debug(f"Component freezing delegated to TrainingStateManager: {components}")

    def unfreeze_components(self, components: List[str]):
        """Unfreeze specified model components (delegated to TrainingStateManager)."""
        # This functionality is now handled by TrainingStateManager
        self.logger.debug(f"Component unfreezing delegated to TrainingStateManager: {components}")

    def update_learning_rates(
        self, optimizer: torch.optim.Optimizer, scale_factor: float
    ):
        """Update learning rates in optimizer (delegated to TrainingStateManager)."""
        # This functionality is now handled by TrainingStateManager
        self.logger.debug(f"Learning rate update delegated to TrainingStateManager: {scale_factor}")

    def validate_configuration(self) -> List[str]:
        """Validate training configuration and return warnings."""
        warnings = []

        # Parameter validation is now handled by TrainingStateManager
        ...
        if coordinate_enabled:
            self.logger.info("Coordinate tokens enabled - will be trained using llm_lr")
    ...
    def _validate_coordinate_token_config(self) -> bool:
        ...
        try:
            ...
            if coordinate_enabled:
                self.logger.info("✅ Coordinate tokens enabled - using llm_lr")
