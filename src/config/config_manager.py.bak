...
from ..logger_utils import get_config_logger
...
class ConfigManager:
    ...
    def _populate_configs(self, config_dict: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.detection = DetectionConfig(
                **extract_config_fields(DetectionConfig, "detection_")
            )
            self.infrastructure = InfrastructureConfig(
                **extract_config_fields(InfrastructureConfig)
            )
        ...
        for field_name in lr_fields:
            if not hasattr(self.training, field_name):
                raise AttributeError(
                    f"Training config missing required field: {field_name}"
                )
    ...
    def _validate_cross_config_dependencies(self) -> None:
        ...
        if not Path(self.data.val_data_path).exists():
            raise ValueError(
                f"Validation data file not found: {self.data.val_data_path}"
            )
        ...
        if (
            self.infrastructure.eval_steps > 0
            and self.infrastructure.eval_strategy == "no"
        ):
    ...
    def get_legacy_config_dict(self) -> Dict[str, Any]:
        ...
        if not self._initialized:
            raise RuntimeError(
                "ConfigManager not initialized. Call load_from_yaml() first."
            )
...
def get_config_manager() -> ConfigManager:
    ...
    if config_manager is None:
        raise RuntimeError(
            "Config manager not initialized. Call init_config_manager() first."
        )
