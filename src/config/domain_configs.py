...
class ModelConfig:
    ...
    def is_flash_attention_enabled(self) -> bool:
        ...
        return (
            self.use_flash_attention and self.attn_implementation == "flash_attention_2"
        )
...
class DataConfig:
    ...
    def validate(self):
        ...
        if self.language not in ["english", "chinese"]:
            raise ValueError(
                f"language must be 'english' or 'chinese', got: '{self.language}'"
            )
        ...
        if self.collator_type not in ["standard", "packed"]:
            raise ValueError(
                f"collator_type must be 'standard' or 'packed', got: '{self.collator_type}'"
            )
...
class DetectionConfig:
    ...
    # Detection system control (DEPRECATED - use coordinate tokens instead)
...
class InfrastructureConfig:
    ...
    def validate(self):
        ...
        if self.eval_steps <= 0 or self.save_steps <= 0 or self.logging_steps <= 0:
            raise ValueError(
                "eval_steps, save_steps, and logging_steps must be positive"
            )
