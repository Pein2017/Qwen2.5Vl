...
from src.models.model_loader import ModelLoadingError, load_model_and_processor_unified
...
class UnifiedBBUTrainer(Trainer):
    ...
    def _validate_training_setup(self):
        ...
        try:
            ...
            if not hasattr(self.model, "detection_enabled"):
                self.logger.warning(
                    "⚠️  Model missing detection_enabled flag - may indicate inconsistent loading"
                )
    ...
    def _compute_loss_with_coordinator(
        ...
        try:
            ...
            self.training_coordinator.step_update(
                current_step=self.state.global_step, current_epoch=self.state.epoch
            )
    ...
    def _compute_loss_legacy(
        ...
        self.logger.warning(
            "⚠️  Using legacy loss computation - teacher-student training may not work properly"
        )
...
def _set_model_training_params(model: nn.Module):
    ...
    logger.info(
        f"📊 Parameter count: {total_params:,} total, {trainable_params:,} trainable"
    )

    # Log detection status
    if hasattr(model, "detection_enabled"):
        logger.info(
            f"🎯 Detection training: {'ENABLED' if model.detection_enabled else 'DISABLED'}"
        )
    ...
    if getattr(config, "use_new_training_system", False):
        try:
            ...
            coordinator = TrainingCoordinator(
                model=model, tokenizer=tokenizer, use_domain_config=False
            )
...
def setup_data_module_unified(
    ...
    eval_dataset = (
        ...
        if getattr(config, "val_data_path", None)
        else None
    )

    # Create data collator
    data_collator = create_data_collator(
        tokenizer=tokenizer, is_packed=getattr(config, "pack_samples", False)
    )
