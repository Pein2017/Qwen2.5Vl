...
def run_integration_tests(verbose: bool = False) -> List[bool]:
    ...
    test_files = [
        "src_new/tests/integration/end_to_end/test_real_data_pipeline.py",
        "src_new/tests/integration/end_to_end/test_complete_pipeline.py",
    ]
...
def run_training_tests(verbose: bool = False) -> List[bool]:
    ...
    test_files = [
        "src_new/tests/unit/training_pipeline/test_loss_computation.py",
        "src_new/tests/unit/training_pipeline/test_token_masking.py",
    ]
...
def run_validation_tests(verbose: bool = False) -> List[bool]:
    ...
    test_files = [
        "src_new/tests/integration/real_data/test_edge_cases.py",
    ]
...
def run_inference_tests(verbose: bool = False) -> List[bool]:
    """Run inference-specific integration tests."""
    logger.info("🖼️ Running Inference Pipeline Tests...")

    test_files = [
        "src_new/tests/integration/inference/test_comprehensive_inference.py",
        "src_new/tests/integration/inference/test_eval_script_simulation.py",
        "src_new/tests/integration/inference/test_path_resolution.py",
    ]
...
def run_all_tests(verbose: bool = False) -> bool:
    ...
    inference_results = run_inference_tests(verbose)
    training_results = run_training_tests(verbose)
    validation_results = run_validation_tests(verbose)

    # Combine all results
    all_results = (
        integration_results + inference_results + training_results + validation_results
    )
...
def main():
    ...
    parser.add_argument(
        "--category",
        choices=["integration", "inference", "training", "validation", "all"],
        default="all",
    ...
    if args.category == "integration":
        ...
        results = run_inference_tests(args.verbose)
        success = all(results)
