...
class TestTokenProcessor:
    ...
    def test_object_wrapping_bbox_2d(self):
        ...
        assert "<|bbox_start|>" in wrapped
        assert "<|bbox_end|>" in wrapped
    ...
    def test_invalid_object_handling(self):
        ...
        assert "<|bbox_start|>" not in wrapped
...
class TestTokenProcessorIntegration:
    ...
    def test_integration_with_objects(self):
        ...
        for obj in test_objects:
            ...
            if "bbox_2d" in obj:
                assert "<|bbox_start|>" in wrapped and "<|bbox_end|>" in wrapped
            elif "square" in obj:
    ...
    def _mock_token_processor(self, config):
        ...
        class MockTokenProcessor:
            ...
            def wrap_object_with_tokens(self, obj):
                desc = obj.get("desc", "")
                if "bbox_2d" in obj:
                    return f"<|obj_ref_start|>{desc}<|obj_ref_end|><|bbox_start|>{obj['bbox_2d']}<|bbox_end|>"
                elif "square" in obj:
