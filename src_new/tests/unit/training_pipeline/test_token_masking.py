...
class TestTokenMaskingWithRealData:
    ...
    def test_conversation_token_structure_logging(self, real_setup):
        ...
        logger.info(f"📝 Testing with student sample: {len(student_sample.get('objects', []))} objects")
        ...
        try:
            ...
            logger.info(f"🎯 Special token IDs: <|im_start|>={im_start_id}, <|im_end|>={im_end_id}, <|image_pad|>={image_pad_id}")
            ...
            image_pad_positions = (input_ids == image_pad_id).nonzero(as_tuple=True)[0].tolist()
            
            logger.info(f"📍 <|im_start|> positions: {im_start_positions}")
            logger.info(f"📍 <|im_end|> positions: {im_end_positions}")
            logger.info(f"🖼️ <|image_pad|> positions: {image_pad_positions[:10]}...")  # First 10
            logger.info(f"🖼️ Total image pad tokens: {len(image_pad_positions)}")
            
            # Analyze conversation segments
            for i, (start_pos, end_pos) in enumerate(zip(im_start_positions, im_end_positions)):
            ...
            # Validate structure
            assert len(im_start_positions) == len(im_end_positions), "Mismatched start/end tokens"
            assert len(im_start_positions) >= 3, "Should have at least system, user, assistant segments"
    ...
    def test_label_masking_with_real_conversation(self, real_setup):
        ...
        try:
            ...
            for i in range(min(2, len(im_start_positions))):
                ...
                segment_text = tokenizer.decode(input_ids[start_pos:end_pos+1], skip_special_tokens=False)
                logger.info(f"  Masked segment {i}: [{start_pos}, {end_pos}] - {segment_text[:50]}...")
            ...
            if len(im_start_positions) >= 3:  # Assistant segment exists
                ...
                assistant_segment = input_ids[assistant_start:assistant_start+10]  # Look at first 10 tokens
                
                # Simple heuristic: mask first few tokens of assistant segment
                labels[assistant_start:assistant_start+3] = -100  # Mask "<|im_start|>assistant\n"
                logger.info(f"  Masked assistant prefix: [{assistant_start}, {assistant_start+3}]")
            ...
            logger.info(f"  Masked tokens (-100): {masked_count} ({masked_count/total_count*100:.1f}%)")
            logger.info(f"  Unmasked tokens: {unmasked_count} ({unmasked_count/total_count*100:.1f}%)")
            
            # Validate masking
            assert masked_count > 0, "Should have some masked tokens"
            assert unmasked_count > 0, "Should have some unmasked tokens for loss computation"
            assert masked_count + unmasked_count == total_count, "All tokens should be accounted for"
