...
class TestPathManager:
    ...
    def test_validate_paths_exist(self, temp_dir, sample_files):
        ...
        paths = ["root_file.txt", "nonexistent_file.txt", "subdir/sub_file.txt"]
...
class TestPathManagerEdgeCases:
    ...
    def test_symlink_handling(self, temp_dir):
        ...
        try:
            ...
            assert resolved.exists()
            # PathManager resolves symlinks to their targets
            assert resolved.name == "original.txt"
...
class TestPathManagerIntegration:
    ...
    def test_double_prefix_prevention_scenario(self, temp_dir):
        ...
        potentially_double_prefixed = str(
            data_root / str(data_root).split("/")[-1] / "test_image.jpg"
        )
