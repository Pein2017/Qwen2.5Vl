...
def test_conversation_flow_structure():
    ...
    try:
        ...
        complete_conversation.append(
            {"role": "system", "content": conversation_flow["system_prompt"]}
        )
        ...
        for teacher in conversation_flow["teacher_examples"]:
            ...
            complete_conversation.append(
                {"role": "user", "content": teacher["user_content"]}
            )
            # Teacher assistant response
            complete_conversation.append(
                {"role": "assistant", "content": teacher["assistant_response"]}
            )
        ...
        print(
            f"   System messages: {sum(1 for msg in complete_conversation if msg['role'] == 'system')}"
        )
        print(
            f"   User messages: {sum(1 for msg in complete_conversation if msg['role'] == 'user')}"
        )
        print(
            f"   Assistant messages: {sum(1 for msg in complete_conversation if msg['role'] == 'assistant')}"
        )
        ...
        assert len(complete_conversation) == expected_messages, (
            f"Expected {expected_messages} messages, got {len(complete_conversation)}"
        )
...
"""
    ...
    print(
        "   2. Teacher example(s) with user prompt + image tokens + assistant response"
    )
    ...
    for component in key_components:
        assert component in inference_prompt_example, (
            f"Missing key component: {component}"
        )
