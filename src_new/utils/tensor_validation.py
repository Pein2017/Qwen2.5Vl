...
Usage:
    from src_new.utils.tensor_validation import TensorValidator, TensorValidationError
...
from .common_imports import torch, np, TORCH_AVAILABLE, NUMPY_AVAILABLE
...
class TensorValidator:
    ...
    def validate_batch_consistency(
        ...
        logger.debug(f"✅ Validated batch consistency: {batch_size} across {list(tensors.keys())}")
        return True
        ...
        if actual_patches != expected_patches:
            ...
            raise TensorValidationError(
                ...
                tensor_info={
                    ...
                    "grid_values": image_grid_thw.tolist() if hasattr(image_grid_thw, 'tolist') else str(image_grid_thw)
                }
            )
...
def validate_tensor_shape(tensor: Any, expected_shape: Tuple[int, ...], name: str = "tensor") -> bool:
