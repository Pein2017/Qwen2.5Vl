...
class PathValidator:
    ...
    def validate_path_exists(
        ...
        if not isinstance(path, (str, Path)):
            raise ValueError(f"{path_type.capitalize()} path must be str or Path, got {type(path)}")
    ...
    @staticmethod
    def validate_file_exists(path: Union[str, Path], allow_missing: bool = False) -> Path:
        """
        ...
        if not allow_missing and validated_path.exists() and not validated_path.is_file():
            raise PathValidationError(
                f"Path exists but is not a file: {validated_path}",
                context={"path": str(validated_path), "is_dir": validated_path.is_dir()}
            )
        
        return validated_path
    
    @staticmethod
    def validate_directory_exists(path: Union[str, Path], allow_missing: bool = False) -> Path:
        """
        ...
        """
        validated_path = PathValidator.validate_path_exists(path, "directory", allow_missing)
        
        if not allow_missing and validated_path.exists() and not validated_path.is_dir():
            raise PathValidationError(
                f"Path exists but is not a directory: {validated_path}",
                context={"path": str(validated_path), "is_file": validated_path.is_file()}
            )
    ...
    def validate_paths_exist(
        ...
        for i, path in enumerate(paths):
            try:
                validated = PathValidator.validate_path_exists(path, path_type, allow_missing)
