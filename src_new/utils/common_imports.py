...
Usage:
    ...
    from src_new.utils.common_imports import *
...
from .logger_factory import get_module_logger, get_debug_logger
from .validation import PathValidator, ValidationError, PathValidationError
from .error_formatting import ErrorMessageBuilder, format_validation_error

# === Type Checking Imports ===
if TYPE_CHECKING:
    from ..config.config import Config
    from ..models.wrapper import DetectionModel
...
def require_torch() -> None:
    ...
    if not TORCH_AVAILABLE:
        raise ImportError(
            "PyTorch is required but not available. "
...
def require_pil() -> None:
    ...
    if not PIL_AVAILABLE:
        raise ImportError(
            "PIL is required but not available. "
...
def require_numpy() -> None:
    ...
    if not NUMPY_AVAILABLE:
        raise ImportError(
            "NumPy is required but not available. "
...
TensorLike = Union[torch.Tensor, np.ndarray] if TORCH_AVAILABLE and NUMPY_AVAILABLE else Any
...
__all__ = [
    ...
    # Project utilities
    "get_module_logger", "get_debug_logger",
    "PathValidator", "ValidationError", "PathValidationError",
    "ErrorMessageBuilder", "format_validation_error",
    
    # Availability checks
    "check_torch_available", "check_pil_available", "check_transformers_available",
    "check_numpy_available", "check_yaml_available",
    "require_torch", "require_pil", "require_transformers", "require_numpy", "require_yaml",
    
    # Constants
    "DEFAULT_LOG_LEVEL", "DEFAULT_LOG_FORMAT",
    "JSONL_EXTENSIONS", "IMAGE_EXTENSIONS", "CONFIG_EXTENSIONS",
    
    # Type aliases
    "PathLike", "TensorLike", "ConfigDict", "SampleDict",
    
    # Utility functions
    "ensure_path", "ensure_list", "safe_import", "get_available_device", "setup_logging",
    
    # Availability flags
    "TORCH_AVAILABLE", "PIL_AVAILABLE", "TRANSFORMERS_AVAILABLE", 
    "NUMPY_AVAILABLE", "YAML_AVAILABLE",
]
