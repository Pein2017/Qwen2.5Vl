...
class CompletionLossComputer:
    ...
    def compute_streaming_loss(
        ...
        if clip_total_tokens > 0:
            clip_ratios["policy_clip_low_ratio"] = float(
                clip_low_tokens
            ) / float(clip_total_tokens)
            clip_ratios["policy_clip_high_ratio"] = float(
                clip_high_tokens
            ) / float(clip_total_tokens)
            clip_ratios["policy_clip_region_ratio"] = float(
                clip_region_tokens
            ) / float(clip_total_tokens)
        else:
    ...
    def _compute_reference_kl(
        ...
        return losses.compute_kl(per_token_logps.detach(), ref_logps)
