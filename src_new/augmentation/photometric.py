...
from src_new.config.augmentation_config import PhotometricConfig
...
def _build_compose(cfg: PhotometricConfig, ocr_guard: bool):
    ...
    extended_ops = safe_ops + [
        A.ImageCompression(
            quality_lower=max(50, int(90 - 40 * m)), quality_upper=95, p=1.0
        ),
        A.GaussNoise(var_limit=(5.0, 40.0 * m), p=1.0),
        A.OneOf(
            [
        ...
        A.CoarseDropout(max_holes=int(4 + 8 * m), max_height=2, max_width=2, p=1.0),
    ]
