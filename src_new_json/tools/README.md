...
- **Usage**: Import from `src_new_json.tools` for development/debugging

### Checkpoint Validator
- **Module**: `checkpoint_validator.py`
- **Purpose**: Validate checkpoint structure and content for inference compatibility
- **Usage**: Import from `src_new_json.tools` or use CLI interface
...
python -m src_new_json.tools.cli validate-checkpoint /path/to/checkpoint

# Validate all checkpoints in a directory
python -m src_new_json.tools.cli validate-all-checkpoints /path/to/output_dir

# Show performance monitor status
python -m src_new_json.tools.cli performance-status

# Reset performance monitor
python -m src_new_json.tools.cli reset-performance
```
...
from src_new_json.tools import PerformanceMonitor, CheckpointValidator
...
These utilities were moved from `src_new_json.utils` to maintain a clean public API. They are no longer exported from the main utils module to prevent accidental use in production code.
