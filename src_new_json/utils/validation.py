...
Usage:
    from src_new_json.utils.validation import PathValidator, ValidationError
...
def _get_src_new_dir() -> Path:
    """Return absolute path to the src_new_json directory."""
    # This file is located at src_new_json/utils/validation.py
    return Path(__file__).resolve().parents[1]  # .../src_new_json
...
def expand_path_aliases(path: Union[str, Path]) -> str:
    ...
    Supported aliases:
    - "@src_new_json" or "@src_new_json/" → absolute path to the repository's src_new_json directory
    """
    ...
    if path_str.startswith("@src_new_json/") or path_str == "@src_new_json":
        src_new_dir = _get_src_new_dir()
        remainder = path_str[len("@src_new_json/") :] if path_str != "@src_new_json" else ""
        expanded = str(src_new_dir / remainder)
        logger.debug(f"Expanded '@src_new_json' alias: {path_str} -> {expanded}")
        return expanded
