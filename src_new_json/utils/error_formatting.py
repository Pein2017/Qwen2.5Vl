...
Usage:
    from src_new_json.utils.error_formatting import ErrorMessageBuilder
...
class ErrorMessageBuilder:
    ...
    def build_file_not_found_error(
        ...
        search_location = (
            Path(search_location) if search_location else missing_path.parent
        )

        error_lines = [f"{file_type.capitalize()} not found: {missing_path}"]
    ...
    def build_directory_structure_error(
        ...
        if available_files:
            error_lines.extend([f"", f"Available files:", f"  {available_files}"])

        if available_dirs:
            error_lines.extend([f"", f"Available directories:", f"  {available_dirs}"])
    ...
    def build_validation_error(
        ...
        """
        error_lines = [f"Validation failed for {item}:"]

        if context:
            error_lines.append(f"Context: {context}")

        error_lines.extend([f"  Expected: {expected}", f"  Actual: {actual}"])
    ...
    def build_tensor_shape_error(
        ...
        """
        error_lines = [f"Tensor shape mismatch for {tensor_name}:"]
        ...
        if len(actual_shape) != len(expected_shape):
            if len(actual_shape) > len(expected_shape):
                suggestions.append(
                    "Consider using squeeze() to remove singleton dimensions"
                )
            else:
                suggestions.append(
                    "Consider using unsqueeze() to add missing dimensions"
                )
    ...
    def build_configuration_error(
        ...
        """
        error_lines = [f"Configuration error for '{config_item}': {issue}"]

        if current_value is not None:
            error_lines.append(f"Current value: {current_value}")

        if valid_options:
            error_lines.extend([f"Valid options: {valid_options}"])

        if config_file:
            error_lines.extend([f"", f"Configuration file: {config_file}"])
    ...
    def build_import_error(
        ...
        if suggestions:
            ...
            error_lines.extend(
                [f"", f"Try installing with:", f"  pip install {module_name}"]
            )
...
def format_tensor_error(name: str, expected_shape: tuple, actual_shape: tuple) -> str:
    ...
    return ErrorMessageBuilder.build_tensor_shape_error(
